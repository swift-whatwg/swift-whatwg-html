...
extension WHATWG.HTML.Attribute {
    ...
    ///     .value("Debug Submit")
    /// ```
    @dynamicMemberLookup public struct FormEncType: WHATWG.HTML.StringAttribute {
        /// The attribute value
...
extension WHATWG.HTML.Attribute.FormEncType {
...
extension WHATWG.HTML.Attribute.FormEncType {
    ...
    public static let urlEncoded = WHATWG.HTML.Attribute.FormEncType(
        contentType: .applicationXWWWFormURLEncoded
    )

    /// Required for file uploads - multipart/form-data (RFC 7578)
    public static let multipartFormData = WHATWG.HTML.Attribute.FormEncType(
        contentType: .multipartFormData()
    )

    /// Minimal encoding, useful for debugging - text/plain
    public static let textPlain = WHATWG.HTML.Attribute.FormEncType(contentType: .textPlain)
}
