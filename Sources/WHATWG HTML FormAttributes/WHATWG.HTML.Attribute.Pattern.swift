...
extension WHATWG.HTML.Attribute {
    ...
    /// ```
    @dynamicMemberLookup public struct Pattern: WHATWG.HTML.StringAttribute {
        /// The attribute value
...
extension WHATWG.HTML.Attribute.Pattern {
...
extension WHATWG.HTML.Attribute.Pattern {
    ...
    public static let username = WHATWG.HTML.Attribute.Pattern("[a-zA-Z0-9]{3,16}")

    /// Common regex pattern for a US phone number (###-###-####)
    public static let usPhone = WHATWG.HTML.Attribute.Pattern("[0-9]{3}-[0-9]{3}-[0-9]{4}")

    /// Common regex pattern for a US zip code (5 digits, optional 4-digit extension)
    public static let usZipCode = WHATWG.HTML.Attribute.Pattern("[0-9]{5}(-[0-9]{4})?")

    /// Common regex pattern for a positive integer
    public static let integer = WHATWG.HTML.Attribute.Pattern("[0-9]+")

    /// Common regex pattern for an alphanumeric string
    public static let alphanumeric = WHATWG.HTML.Attribute.Pattern("[a-zA-Z0-9]+")
}
