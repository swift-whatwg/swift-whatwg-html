...
extension WHATWG.HTML.Attribute {
    ...
    /// ```
    @dynamicMemberLookup public struct Action: WHATWG.HTML.StringAttribute {
...
extension WHATWG.HTML.Attribute.Action {
...
extension WHATWG.HTML.Attribute.Action {
    ...
    public static func relative(_ path: String) -> WHATWG.HTML.Attribute.Action {
        // Make sure the path starts with a slash if it's not already
        let formattedPath = path.hasPrefix("/") ? path : "/\(path)"
        return WHATWG.HTML.Attribute.Action(formattedPath)
    }

    /// Creates an absolute URL action to an external service
    public static func absolute(_ url: String) -> WHATWG.HTML.Attribute.Action {
        return WHATWG.HTML.Attribute.Action(url)
    }

    /// Creates an action pointing to the current page
    public static let current: WHATWG.HTML.Attribute.Action = WHATWG.HTML.Attribute.Action(".")

    /// Creates an action for client-side handling (no server submission)
    public static let clientSide: WHATWG.HTML.Attribute.Action = WHATWG.HTML.Attribute.Action("#")
}
