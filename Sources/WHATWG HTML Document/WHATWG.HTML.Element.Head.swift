...
extension WHATWG.HTML.Element {
    ...
    ///
    public struct Head: WHATWG.HTML.Element.`Protocol` {
        /// Creates a new Head element with the specified content.
        public init() {}
    }
}

extension WHATWG.HTML.Element.Head {
    /// The HTML tag name
    @inlinable public static var tag: String { "head" }
    public static let categories: Set<WHATWG.HTML.Element.Content.Category> = []
    public static let content: WHATWG.HTML.Element.Content = .init(model: .categories([.metadata]))
}
