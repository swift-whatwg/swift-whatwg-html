...
extension WHATWG.HTML.ExternalObject.`Type` {
    ...
    /// ```swift
    /// // PDF object
    /// HTML.object.data("document.pdf").type(HTML.ExternalObject.`Type`.Attribute(contentType: .applicationPDF))
    ///
    /// // Video object
    /// HTML.object.data("video.mp4").type(HTML.ExternalObject.`Type`.Attribute(contentType: .videoMP4))
    ///
