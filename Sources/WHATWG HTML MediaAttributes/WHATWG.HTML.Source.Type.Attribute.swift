...
extension WHATWG.HTML.Source.`Type` {
    ...
    /// HTML.source.src("video.mp4").type(HTML.Source.`Type`.Attribute(contentType: .videoMP4))
    ///
    /// // Audio source
    /// HTML.source.src("audio.mp3").type(HTML.Source.`Type`.Attribute(contentType: .audioMPEG))
    ///
    /// // Image source for picture element
    /// HTML.source.srcset("image.webp").type(HTML.Source.`Type`.Attribute(contentType: .imageWEBP))
    ///
