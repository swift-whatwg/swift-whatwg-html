...
@Suite struct `Bring Attention To Tests` {
    ...
    @Test func `Basic B element creation`() {
        let b = WHATWG.HTML.Element.B()
        #expect(WHATWG.HTML.Element.B.tag == "b")
    }

    @Test func `B element conforms to HTMLElement`() {
        let b = WHATWG.HTML.Element.B()
        let element: any WHATWG.HTML.Element.`Protocol` = b
    ...
    @Suite struct `Integration Tests` {
        ...
        @Test func `Multiple B elements`() {
            let b1 = WHATWG.HTML.Element.B()
            let b2 = WHATWG.HTML.Element.B()
            let b3 = WHATWG.HTML.Element.B()
        ...
        @Test func `Nested B elements`() {
            let outerB = WHATWG.HTML.Element.B()
            let innerB = WHATWG.HTML.Element.B()

            #expect(WHATWG.HTML.Element.B.tag == "b")
            #expect(outerB is any WHATWG.HTML.Element.`Protocol`)
            #expect(innerB is any WHATWG.HTML.Element.`Protocol`)
        }
    }
