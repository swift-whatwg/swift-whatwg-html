...
@Suite struct `Bidirectional Isolate Tests` {
    ...
    @Test func `Basic BDI element creation`() {
        let bdi = WHATWG.HTML.Element.BidirectionalIsolate()
        #expect(WHATWG.HTML.Element.BidirectionalIsolate.tag == "bdi")
    }

    @Test func `BDI element conforms to HTMLElement`() {
        let bdi = WHATWG.HTML.Element.BidirectionalIsolate()
        let element: any WHATWG.HTML.Element.`Protocol` = bdi
    ...
    @Suite struct `Integration Tests` {
        ...
        @Test func `Multiple BDI elements`() {
            let bdi1 = WHATWG.HTML.Element.BidirectionalIsolate()
            let bdi2 = WHATWG.HTML.Element.BidirectionalIsolate()
            let bdi3 = WHATWG.HTML.Element.BidirectionalIsolate()
        ...
        @Test func `Nested text direction contexts`() {
            let outerBdi = WHATWG.HTML.Element.BidirectionalIsolate()
            let innerBdi = WHATWG.HTML.Element.BidirectionalIsolate()

            #expect(WHATWG.HTML.Element.BidirectionalIsolate.tag == "bdi")
            #expect(outerBdi is any WHATWG.HTML.Element.`Protocol`)
            #expect(innerBdi is any WHATWG.HTML.Element.`Protocol`)
        }
