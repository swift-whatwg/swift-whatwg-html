...
@Suite struct `Dirname Test` {
    @Test func `Dirname attribute should be dirname`() {
        #expect(WHATWG.HTML.Attribute.Dirname.attribute == "dirname")
    }

    @Test func `Dirname should support initialization based on element name`() {
        let dirname = WHATWG.HTML.Attribute.Dirname(basedOn: "comment")
        #expect(dirname.rawValue == "comment-direction")

        let dirnameCustomSuffix = WHATWG.HTML.Attribute.Dirname(basedOn: "message", suffix: "-dir")
