...
@Suite struct `Download Test` {
    @Test func `Download attribute should be download`() {
        #expect(WHATWG.HTML.Attribute.Download.attribute == "download")
    }

    @Test func `Download boolean literal support`() {
        let downloadTrue: WHATWG.HTML.Attribute.Download = true
        let downloadFalse: WHATWG.HTML.Attribute.Download = false
    ...
    @Test func `Download with filename`() {
        let download = WHATWG.HTML.Attribute.Download("report.pdf")
