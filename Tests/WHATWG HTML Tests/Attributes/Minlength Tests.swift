...
@Suite struct `Minlength Test` {
    @Test func `Minlength attribute should be minlength`() {
        #expect(WHATWG.HTML.Attribute.Minlength.attribute == "minlength")
    }

    @Test func `Minlength should support integer literal`() {
        let minlength: WHATWG.HTML.Attribute.Minlength = 5
    ...
    @Test func `Minlength convenience properties and methods`() {
        #expect(WHATWG.HTML.Attribute.Minlength.none.value == 0)
        #expect(WHATWG.HTML.Attribute.Minlength.characters(10).value == 10)
    }
}
