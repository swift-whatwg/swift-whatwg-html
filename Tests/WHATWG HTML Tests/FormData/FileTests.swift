...
@Suite struct `File Tests` {
    ...
    @Test func `File conforms to Hashable`() {
        let data = [UInt8]([1, 2, 3])
        let file1 = WHATWG_HTML.Attribute.Form.Data.File(
            name: "test.txt",
            type: "text/plain",
            body: data
        )
        let file2 = WHATWG_HTML.Attribute.Form.Data.File(
            name: "test.txt",
            type: "text/plain",
            body: data
        )
    ...
    @Test func `MIME type constants are correct`() {
        ...
        #expect(
            WHATWG_HTML.Attribute.Form.Data.File.MIMEType.octetStream == "application/octet-stream"
        )
    }
